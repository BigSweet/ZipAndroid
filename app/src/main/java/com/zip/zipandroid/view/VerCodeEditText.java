...
public class VerCodeEditText extends androidx.appcompat.widget.AppCompatEditText implements
    ...
    private int mUnSelectedBackgroundColor;//未选中的背景颜色
    private int backgroundRadius; //背景部分的圆角
    ...
    private void initPaint() {
        ...
        mNormalBackgroundPaint.setColor(mUnSelectedBackgroundColor);
    ...
    private void initAttrs(AttributeSet attrs) {
        ...
        backgroundRadius = (int) ta.getDimension(R.styleable.VerCodeEditText_verBackgroundRadius, 0);
        ...
        mSelectedBackgroundColor = ta.getColor(R.styleable.VerCodeEditText_selectedBackgroundColor,
                getColor(android.R.color.darker_gray));
        mUnSelectedBackgroundColor = ta.getColor(R.styleable.VerCodeEditText_selectedBackgroundColor,
    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i < mFigures; i++) {
            ...
            int end = width + start;
            RectF rectF = new RectF(start, 0, end, height);
            //画一个矩形
            if (i == mCurrentPosition) {//选中的下一个状态
                canvas.drawRoundRect(rectF, backgroundRadius, backgroundRadius, mSelectedBackgroundPaint);
            } else {
                canvas.drawRoundRect(rectF, backgroundRadius, backgroundRadius, mNormalBackgroundPaint);
            }
            canvas.restore();
        }
        //绘制文字
    ...
    public void afterTextChanged(Editable s) {
        ...
        if (getText().length() == mFigures) {
            ...
            if (onCodeChangedListener != null) {
                onCodeChangedListener.onInputClear();
            }
        }
    }
    ...
    @Override
    public void setUnSelectedBackgroundColor(int selectedBackground) {
        mUnSelectedBackgroundColor = getColor(selectedBackground);
        mNormalBackgroundPaint.setColor(mUnSelectedBackgroundColor);
        postInvalidate();
    }
